...
// --- Lead & Client ---
export type LeadStatus = 'New Lead' | 'Contacted' | 'Audit in Progress' | 'Proposal Sent' | 'Negotiation' | 'Closed Won' | 'Closed Lost';
export const leadStatuses: LeadStatus[] = ['New Lead', 'Contacted', 'Audit in Progress', 'Proposal Sent', 'Negotiation', 'Closed Won', 'Closed Lost'];
export const allLeadStatusesForSystem: LeadStatus[] = ['New Lead', 'Contacted', 'Audit in Progress', 'Proposal Sent', 'Negotiation', 'Closed Won', 'Closed Lost'];
export const KANBAN_STAGE_ORDER: LeadStatus[] = ['New Lead', 'Contacted', 'Audit in Progress', 'Proposal Sent', 'Negotiation', 'Closed Won'];
...
export interface Lead {
    ...
    // Extended Digital Marketing Agency Insights
    primaryGoal?: string;
...
// --- Projects & Tasks ---
export type ProjectStatus = 'Backlog' | 'Planning' | 'In Progress' | 'Review' | 'Blocked' | 'Done' | 'Completed' | 'On Hold' | 'Cancelled';
...
export interface Invoice {
    ...
    parentOccurrenceDate?: string; // ISO
...
// --- B2B / General Audit Specific Structures (New) ---
export interface B2BAuditSection {
    scores: Record<string, number>; // e.g., { "seoRank": 6, "ctaClarity": 7 }
    issues: string;
...
export interface B2BAuditData {
    leadGen: B2BAuditSection; // Lead Generation & Website SEO
    salesFunnel: B2BAuditSection; // Sales Funnel & Nurture
    adsOutbound: B2BAuditSection; // Ads & Outbound Channels
    offerPositioning: B2BAuditSection; // Offer & Consultative Fit
    crmFollowUp: B2BAuditSection; // CRM & Follow-up Speed
    techStack: B2BAuditSection; // B2B Tech & Tracking
    summary: B2BAuditSection;
}
...
export interface Audit {
    ...
    b2bAuditData?: B2BAuditData; // Complex Structured General/Other (B2B/SaaS/Service) data
    brandAuditType?: 'D2C' | 'B2B_Other'; // Differentiates the layout used
}
