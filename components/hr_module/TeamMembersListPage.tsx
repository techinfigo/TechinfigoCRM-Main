...
import { TeamMember, HRStatus, FeatureKey, PermissionAction, RoleDefinition } from '../../types'; // Ensured relative path
import { Button } from '../common/Button'; // Ensured relative path
import { Card } from '../common/Card'; // Ensured relative path
...
export const TeamMembersListPage: React.FC<TeamMembersListPageProps> = ({
  ...
  const TableHeader: React.FC<{ sortKey: SortableKey; label: string; }> = ({ sortKey, label }) => (
    <th 
        scope="col" 
        className="px-4 py-3 text-left text-xs font-semibold text-text-muted dark:text-text-muted uppercase tracking-wider group"
    ...
    >
      {teamMembers.length === 0 ? (
        <div className="text-center py-12 flex-grow flex flex-col items-center justify-center">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-16 h-16 text-slate-400 dark:text-slate-500 mb-4"><path d="M10.5 6a7.5 7.5 0 100 15h3a7.5 7.5 0 100-15h-3z" /><path d="M10.5 9a2.25 2.25 0 100 4.5 2.25 2.25 0 000-4.5zM13.5 4.5v.001M13.5 19.5v.001" /></svg>
          <p className="text-slate-500 dark:text-slate-400 mt-2 text-lg">No team members found in HR Module.</p>
          {canManageHRMembers && (
            <p className="text-slate-400 dark:text-slate-500 text-sm">Get started by adding your first team member for HR records.</p>
          )}
        </div>
      ) : (
