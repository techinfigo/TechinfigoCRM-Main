...
export const EmployeeDetailView: React.FC<EmployeeDetailViewProps> = ({ 
  ...
  const handleDownloadDocument = (doc: UploadedHRDocument) => {
    if (doc.file) {
      ...
      alert(`No file is attached to "${doc.name}" yet. Documents you upload here can be downloaded; sample/pre-loaded entries have no file.`);
    }
  };

  const handleDownloadPayslip = () => {
    if (!calculatedPayrollData) {
      alert('Please calculate the payroll for a month first.');
      return;
    }
    const monthLabel = new Date(selectedPayrollMonth + '-02').toLocaleString('default', { month: 'long', year: 'numeric' });
    const d = calculatedPayrollData;
    const rows = [
      ['Base Monthly Salary', `₹${d.baseSalary.toLocaleString('en-IN')}`],
      ['Total Days in Month', String(d.totalDaysInMonth)],
      ['Assumed Working Days', String(d.workingDays)],
      ['Days Present (Payable)', String(d.daysPresent)],
      ['Days Absent (Unpaid)', String(d.daysAbsentUnpaid)],
    ].map(([k, v]) => `<tr><td style="padding:6px 10px;border:1px solid #ddd">${k}</td><td style="padding:6px 10px;border:1px solid #ddd;text-align:right">${v}</td></tr>`).join('');
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Payslip - ${employee.name} - ${monthLabel}</title></head>
      <body style="font-family:Arial,sans-serif;max-width:600px;margin:24px auto;color:#111">
        <h2 style="margin:0 0 4px">Payslip</h2>
        <p style="margin:0 0 2px;color:#555">${monthLabel}</p>
        <hr style="margin:12px 0;border:none;border-top:2px solid #0C2B2B"/>
        <p style="margin:2px 0"><strong>Employee:</strong> ${employee.name}</p>
        <p style="margin:2px 0"><strong>Role:</strong> ${employee.role || '-'}</p>
        <table style="border-collapse:collapse;width:100%;margin-top:14px">${rows}
          <tr><td style="padding:8px 10px;border:1px solid #ddd;font-weight:bold">Net Payable Salary</td>
          <td style="padding:8px 10px;border:1px solid #ddd;text-align:right;font-weight:bold;color:#0a7a35">₹${d.netPayableSalary.toLocaleString('en-IN')}</td></tr>
        </table>
        <p style="margin-top:24px;color:#888;font-size:12px">Generated by Techinfigo CRM. This is a system-generated payslip.</p>
      </body></html>`;
      ...
      case 'Payroll':
        return (
            <Card title="Payroll Information" className="bg-transparent shadow-none border-0 p-0">
                ...
                <Button variant="outline" size="xs" className="mt-3" onClick={handleDownloadPayslip} disabled={!calculatedPayrollData}>Download Full Payslip (PDF)</Button>
            </Card>
        );
      case 'Performance':
        return (
